...
class DiagGaussianMixturePdType(PdType):
    def __init__(self, size, n):
        self.size = size
        self.n = n
    def pdclass(self):
        return DiagGaussianMixturePd
    def param_shape(self):
        return [self.n + 2*self.size*self.n]
    def sample_shape(self):
        return [self.size]
    def sample_dtype(self):
        return tf.float32
    def pdfromflat(self, flat):
        return self.pdclass()(flat, self.n)
...
class DiagGaussianMixturePd(Pd):
    def __init__(self, flat, n, hard_grad=False):
        self.n = n
    ...
    def sample(self):
        ...
        return tf.gather(samples, self.mixture.sample())[0]
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
